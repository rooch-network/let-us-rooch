...
***
### 第四次作业
1.Apots的话使用的是不同的ref来控制不同的权限，需要删除的话需要鲜拿到相印的ref才能进行删除，Rooch主要是通过传入一个Object的指针来进行的
2.sui的object的一个实现，它默认的话，如果你的一个struct的对你的结构体里面，你的第一个字段定义的是一个UID类型的话，他的系统就会认为struct是一个object的类型，所以sui的保障是在vm层面，通过UID来进行对象的一个删除
3.sui的object的话，它是系统层面就是它的web层面来保障和创建的,Rooch里面的话，是在合约里面有一套完整的定义，不依赖于vm的保障
