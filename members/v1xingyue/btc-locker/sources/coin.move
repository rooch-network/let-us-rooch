...
module btc_locker::holder_coin {
    ...
    use bitcoin_move::types::{tx_output,txout_script_pubkey};
    use bitcoin_move::script_buf::{is_op_return,witness_program};
    ...
    const ErrorUTXONotSpecial: u64 = 3;
    ...
    public fun special_claim(coin_info_holder_obj: &mut Object<CoinInfoHolder>, utxo_obj: &mut Object<UTXO>): Coin<HDC> {
        let myutxo = object::borrow(utxo_obj);
        assert!(utxo_special(myutxo), ErrorUTXONotSpecial);
        let utxo_value = utxo::value(myutxo);
        let coin_info_holder = object::borrow_mut(coin_info_holder_obj);
        let mint_amount = ( utxo_value * 1000 as u256);
        let coin = coin::mint_extend(&mut coin_info_holder.coin_info, mint_amount);
        coin
    }

    public entry fun claim_special(coin_info_holder_obj: &mut Object<CoinInfoHolder>, utxo: &mut Object<UTXO>) {
        let coin = special_claim(coin_info_holder_obj, utxo);
        let sender = tx_context::sender();
        account_coin_store::deposit(sender, coin);
    }
